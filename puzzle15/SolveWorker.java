...
public class SolveWorker extends SwingWorker<Void, int[][]> {
    ...
    private int rows = 4;
    private int cols = 4;
    private Puzzle4x4 puzzle4x4Instance;

    public SolveWorker(int[][] initialState, Puzzle4x4 puzzle4x4Instance) {
        this.initialState = initialState;
        this.puzzle4x4Instance = puzzle4x4Instance;
    }
...
protected void process(List<int[][]> chunks) {
    for (int[][] boardState : chunks) {
        for (int i = 0; i < puzzle4x4Instance.rows; i++) {
            for (int j = 0; j < puzzle4x4Instance.cols; j++) {
                puzzle4x4Instance.board[i][j] = boardState[i][j];
            }
        }
        puzzle4x4Instance.updateBoard(); // Gọi phương thức updateBoard() của Puzzle4x4
    }
}
